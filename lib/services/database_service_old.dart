...
class DatabaseService {
  ...
  // ============================================================
  // ACTIVATE NEXT SEED FOR FLOWER TYPE
  // ============================================================
  ...
  Future<Map<String, Object?>?> activateNextSeedForFlower({
    ...
    return db.transaction((txn) async {
      final active = await txn.query(
        'user_flowers',
        where: '''
          user_id = ? AND flower_id = ?
          AND is_active = 1 AND is_bloomed = 0
        ''',
        whereArgs: [userId, flowerId],
        orderBy: 'id ASC',
        limit: 1,
  ...
  Future<Map<String, int>> getUnusedSeedCounts(String userId) async {
    ...
    return {
      for (final row in rows)
        row['flower_id'].toString(): (row['count'] as num?)?.toInt() ?? 0,
    };
  }

  // ============================================================
  // BLOOM COUNTS BY FLOWER
  // ============================================================
  ...
  // ============================================================
  // CURRENT ACTIVE FLOWERS BY TYPE
  // ============================================================
  ...
  Future<Map<String, Map<String, Object?>>> getActiveFlowersByType(
    ...
    for (final row in rows) {
      result.putIfAbsent(row['flower_id'].toString(), () => row);
    }
    return result;
  }
